...
const state: AppState = {
  ...
  editorMode: (window.localStorage.getItem("jtype.ui.editorMode") as EditorMode | null) ?? "split",
  currentPath: "",
  currentRelativePath: "",
  currentKind: "",
  originalContent: "",
...
function publicUrlFor(relativePath: string) {
  ...
  const binding = currentVaultBinding();
  if (!binding) return "";
  const cloudWs = state.cloudWorkspaces.find((w) => w.id === binding.workspaceId);
  ...
  return `${state.syncSiteUrl.replace(/\/$/, "")}/${wsSlug}/${withoutExt}`;
}
...
function setEditorMode(mode: EditorMode) {
  ...
  window.localStorage.setItem("jtype.ui.editorMode", mode);
...
function toggleDocumentInfo() {
  ...
  window.localStorage.setItem("jtype.ui.documentPanelOpen", JSON.stringify(!document.body.classList.contains("document-panel-collapsed")));
}

function toggleFocusMode() {
  document.body.classList.toggle("focus-mode");
  window.localStorage.setItem("jtype.ui.focusMode", JSON.stringify(document.body.classList.contains("focus-mode")));
...
if (window.localStorage.getItem("jtype.ui.documentPanelOpen") === "false") document.body.classList.add("document-panel-collapsed");
if (window.localStorage.getItem("jtype.ui.focusMode") === "true") document.body.classList.add("focus-mode");
updateActionButtons();
renderRecentItems();
renderFavoriteItems();
renderSyncSession();
