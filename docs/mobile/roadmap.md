...
> 当前阶段：Phase 2D — 系统集成与可靠性；系统分享、recovery、无障碍、共享触控交互、键盘辅助栏、5,000 文档大 vault、大内容/Board、sync reliability、系统通知 → 文档定位，以及 mobile partial workspace runtime/loaded-native fallback/folder hydration 已完成；Android partial 5,008-entry 与 iOS clean-static 5,406-entry cold open/tail restore/交互式第二页已通过，连续 shallow page 的双端 native cache hit 为 0 ms。external reconcile/write-back 已改为 native full scan + plan-driven source materialization，unchanged 稳定状态已复用 trusted baseline 消除重复 mirror full hash；HTTPS universal/app-link、APNs device-token / FCM FID 注册、服务端 durable outbox、FCM/APNs credential transport、重试、invalid-registration cleanup、provider collapse/TTL、无凭据 maintenance/retention，以及 native durable refresh hint → shared sync recovery 已完成工程及 Simulator gate。Android Studio 已默认选择 arm64 flavor；Xiaomi Android 16 真机现已 preflight READY，并完成 arm64 APK 安装、261 ms cold launch、shared UI 与真实 `[JTYPE]` Launcher icon gate。iOS 真机/签名仍 blocked；继续生产关联部署、真实 provider credentials/network/physical background delivery、release signing、source full-hash/provider-native streaming、双平台 physical low-memory/performance、真实设备弱网与真机终验
...
4. **2D 系统集成与可靠性（约 1–2 周）**：share target、pending OAuth 冷恢复、无障碍、草稿恢复、大 vault/弱网测试；5,000 文档共享索引、大 Markdown/附件/1,200-card Board 渐进渲染、sync batching/retry/idempotency/服务中断恢复、双平台原生通知文档定位，以及 APNs device-token / FCM FID 注册、durable outbox、FCM/APNs credential transport、重试、失效 registration 清理、provider collapse/TTL、四小时 hint retention 和 content-free native hint → shared sync recovery 已通过工程/模拟器 gate；external reconcile/write-back 的选择性 source materialization 已完成工程 gate，继续真实 provider credentials/network/physical background delivery、physical low-memory、真实设备弱网与双平台真机 gate。
...
- [~] 本地原生协作通知、按需权限、Android channel 与双平台 tap callback 已接入；`e132c0f` 完成 APNs device-token / FCM FID 注册 adapter、authenticated registration/rotation/logout API、canonical payload 与 injected notification route；`611dbcd` 完成 active-member durable outbox、固定官方 FCM/APNs transport、重试和 invalid-registration cleanup；`948ecf6` 增加单一 provider collapse ID、一小时 TTL/expiration、无凭据 maintenance 与四小时私有 hint retention；`c14571a` 再把 canonical push 收口为 content-free native pending bit，由根 `src/` 的 shared lifecycle consume/coalesce 并调用既有 sync，iOS visible APNs alert 同时携带 `content-available=1`。真实 provider credentials/network、release signing、physical-device delivery 和系统实际授予的 background/terminated callback 待完成；后台不得复制 shared sync 或宣称系统保证投递（证据：`docs/mobile/reports/phase-2-notification-routing.md`、`docs/mobile/reports/phase-2-remote-push.md`、`docs/mobile/reports/phase-2-provider-transport.md`、`docs/mobile/reports/phase-2-push-retention.md`、`docs/mobile/reports/phase-2-push-refresh.md`，实现：`2a1fc09`、`ff21f86`、`999dc11`、`e132c0f`、`611dbcd`、`948ecf6`、`c14571a`）
...
- [~] Android/iOS share smoke flow、OAuth deep-link、custom/HTTPS canonical 文档 route 与双平台原生通知点击定位已通过；Universal/App Link、APNs/FCM identifier registration、durable outbox、credential transport、retry、invalid-registration cleanup、provider collapse/TTL、hint retention 和 native pending hint → shared sync recovery 的工程/Simulator gate 已完成，生产 verified association、真实 provider credentials/network/physical background delivery、release signing 与真实设备矩阵待完成
...
- [x] 当前 2D provider on-demand 增量的原始 plugin cargo check、Tauri 29/29、unit 59/59、app E2E 55/55 与双平台构建均通过；`264db8a` follow-up 又通过 unit 73/73、app E2E 56/56、jtype-core 46/46、Tauri 29/29、Desktop/Android/iOS build，以及 Android/iOS 120-file `1 changed / 0 changed` 原生交互/日志 gate（证据：`docs/mobile/reports/phase-2-native-on-demand.md`）
...
- [x] physical-device preflight 工程 gate 已完成：Android/iOS 可独立或联合 fail-closed 检查，Emulator/Simulator 只作诊断，设备标识在 Markdown/JSON 中脱敏。当前 Android-only gate 已在 Xiaomi Android 16 / API 36 / arm64 真机变为 READY，并完成安装、cold launch 与 Launcher icon；iOS 仍因 0 physical iPhone、0 Apple Development identity 和未配置 team blocked（证据：`docs/mobile/reports/phase-2-physical-device-preflight.md`、`docs/mobile/reports/phase-2-android-physical-install.md`，实现：`c63aac3`、`1d29199`）
...
- [x] 当前 push pressure/retention 增量通过 unit 85/85、app E2E 56/56、Tauri 30/30、jtype-core 46/46、web service 316/316、push unit 7/7、push API 4/4、Docker config、Desktop build、Android final APK 与 iOS static archive；无 provider 的过期/撤员清理、FCM/APNs collapse/TTL contract 与双端 shared EditorShell 均通过。真实后台/provider/physical delivery 明确保留（证据：`docs/mobile/reports/phase-2-push-retention.md`，实现：`948ecf6`）
- [x] 当前 push refresh-hint 增量通过 unit 86/86、app E2E 56/56、target recovery E2E 1/1、Tauri 30/30、jtype-core 46/46、web service 316/316、Docker config、Desktop build、Android universal APK/AAB 与 iOS static archive；Android/iOS native 只保存 content-free bit，consume/coalescing 复用 shared sync，双端继续显示 Desktop 共用 EditorShell。真实凭据与 physical background/terminated delivery 明确保留（证据：`docs/mobile/reports/phase-2-push-refresh.md`，实现：`c14571a`）
- [~] 双平台模拟器截图已持续保存；Android physical install/shared UI/Launcher icon 截图已完成，完整 Android physical reliability recording 与 iOS physical screenshot/recording 待后续真机矩阵
- [~] `docs/mobile/reports/phase-2.md` 已记录 2A、2B、2C、2D recovery、系统分享、无障碍、触控交互、大 vault、大内容、sync reliability、通知文档定位、remote push registration contract、shared push refresh recovery、Android physical install/icon 与 partial workspace runtime；专项报告后续持续更新到 Phase 2 终验
- [~] tracking 已记录当前 Phase 2 commit hashes（最新实现 gate `1d29199`）；后续增量继续追加
...
| 2026-07-19 | 2.3 / 2D | `948ecf6` | provider-independent queue maintenance、四小时私有 hint retention、FCM/APNs collapse 与一小时 provider TTL | `docs/mobile/reports/phase-2-push-retention.md` |
| 2026-07-19 | 2.3 / 2D | `c14571a` | Android/iOS content-free durable refresh bit、iOS visible `content-available` hook 与根 `src/` shared sync recovery consume/coalescing | `docs/mobile/reports/phase-2-push-refresh.md` |
| 2026-07-19 | 2.5 / physical | `1d29199` | generated Android launcher/adaptive/round icon 同步 canonical `[JTYPE]`，Xiaomi Android 16 arm64 安装、cold launch 与真实 Launcher render | `docs/mobile/reports/phase-2-android-physical-install.md` |
