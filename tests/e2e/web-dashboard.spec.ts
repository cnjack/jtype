...
test.describe("Workspace Documents", () => {
  ...
  test("deletes a document", async ({ page }) => {
    // Click the delete button (×) for intro.md
    const introRow = page.locator("text=intro.md").first();
    await introRow.click();
    await page.locator("text=intro.md").locator("..").getByRole("button", { name: "×" }).click();
    // After delete, list is refreshed (mock returns same list since we can't track state)
    await expect(page.getByText("intro.md")).toBeVisible();
  });
});
...
test.describe("Admin Panel", () => {
  ...
  test("shows stats overview", async ({ page }) => {
    await expect(page.getByText("5")).toBeVisible(); // Users
    await expect(page.getByText("12")).toBeVisible(); // Workspaces
    await expect(page.getByText("89")).toBeVisible(); // Documents
  });
