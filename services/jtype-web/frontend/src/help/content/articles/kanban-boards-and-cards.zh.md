...
一个**看板**属于某一个工作区，用来归类相关的工作——路线图、迭代、内容日历等等。每个新建的看板都自带三个状态：**To do**、**Doing** 和 **Done**，显示为纵向泳道。你可以重命名、改颜色、调整状态顺序，还能给每个状态设置一个可选的 WIP（在制品）上限作为温和提示。这个上限仅供参考——JType 永远不会因此阻止你再加一张卡片。
