...
pub fn build_router(pool: Pool<MySql>, public_base_url: String) -> Router {
    ...
    Router::new()
        ...
        // User profile API
        .route("/api/me/profile", get(handlers::user::get_profile).put(handlers::user::update_profile))
        ...
        .route("/api/admin/users/:user_id", get(handlers::admin::get_user).put(handlers::admin::update_user))
        .route("/api/admin/workspaces", get(handlers::admin::list_workspaces))
        ...
        .route("/api/v1/workspace-invites/:invite_token/accept", post(handlers::workspace::accept_invite))
        // Documents API
        .route("/api/v1/workspaces/:workspace_id/documents", get(handlers::document::list_documents).put(handlers::document::save_document))
        .route("/api/v1/workspaces/:workspace_id/documents/:document_id", get(handlers::document::get_document).delete(handlers::document::delete_document))
        .route("/api/v1/workspaces/:workspace_id/documents/:document_id/status", put(handlers::document::update_status))
        .route("/api/v1/workspaces/:workspace_id/documents/:document_id/versions", get(handlers::document::list_versions))
        // Sync API
        .route("/api/v1/workspaces/:workspace_id/sync/pull", post(handlers::sync::pull))
...
mod tests {
    ...
    #[test]
    fn hides_drafts() {
        assert_eq!(normalize_status("published", "---\nstatus: draft\n---\n# A"), "draft");
