...
pub async fn update_user(
    ...
    if let Some(budget) = payload.storage_budget_bytes {
        if budget < 0 {
            return Err(AppError::BadRequest("budget must be non-negative".to_string()));
        }
...
pub async fn stats(
    ...
    let documents: i64 = sqlx::query("SELECT COUNT(*) AS c FROM documents")
        ...
        .await?
        .try_get("c")?;
    let storage: i64 = sqlx::query("SELECT COALESCE(SUM(OCTET_LENGTH(content)), 0) AS c FROM documents")
        .fetch_one(&state.pool)
        .await?
        .try_get("c")?;
