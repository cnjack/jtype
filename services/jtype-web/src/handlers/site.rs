...
fn select_document<'a>(
    ...
    let Some(page_path) = page_path.filter(|v| !v.is_empty()) else {
        return docs
            .iter()
            .find(|d| d.relative_path == "index.md")
            .or_else(|| docs.first());
    };
    ...
    docs.iter()
        .find(|d| candidates.iter().any(|c| c == &d.relative_path))
}
...
fn site_shell(
    ...
    let nav = build_nav_tree(docs, username, workspace_slug, active_path);
    let doc_count = docs.len();

    format!(
        r#"<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{}</title><style>{}</style><script src="https://cdn.jsdelivr.net/npm/mermaid@10/dist/mermaid.min.js"></script><script>mermaid.initialize({{startOnLoad:true,securityLevel:'strict',theme:'neutral'}});</script></head><body><aside><div class="sidebar-header"><div class="brand"><span class="brand-bracket">[</span><span class="brand-j">J</span><span class="brand-type">TYPE</span><span class="brand-bracket">]</span></div><div class="site-title">{}</div><span class="doc-count">{} document{}</span></div><nav class="sidebar-nav">{}</nav></aside><main><article class="prose">{}</article></main></body></html>"#,
...
fn insert_into_tree<'a>(
    ...
    if let NavNode::Folder { children, .. } = &mut nodes[idx] {
        insert_into_tree(children, &parts[1..], doc);
    }
}
    ...
    for node in nodes {
        match node {
            NavNode::Folder { name, children } => {
                let folder_prefix = find_folder_prefix(children);
                let is_open = active_path.starts_with(&folder_prefix);
                ...
                html.push_str(&format!(
                    r#"<li class="nav-folder"><details{}><summary class="nav-folder-name" style="padding-left:{}rem">{}{}</summary>{}</details></li>"#,
                    if is_open { " open" } else { "" },
                    0.625 + depth as f64 * 0.875,
                    chevron,
                    escape_html(name),
                    render_nav_nodes(children, username, workspace_slug, active_path, depth + 1),
                ));
            }
            NavNode::Doc { doc } => {
                ...
                html.push_str(&format!(
                    r#"<li><a class="nav-link{}" style="padding-left:{}rem" href="{}">{}</a></li>"#,
                    active,
                    0.625 + depth as f64 * 0.875,
                    page_href(username, workspace_slug, &doc.relative_path),
                    escape_html(label),
                ));
            }
        }
...
fn find_folder_prefix(children: &[NavNode<'_>]) -> String {
    // Get the common path prefix from any doc in this folder subtree
    for child in children {
        match child {
            NavNode::Doc { doc } => {
                if let Some(pos) = doc.relative_path.rfind('/') {
                    return format!("{}/", &doc.relative_path[..pos]);
                }
                return String::new();
            }
            NavNode::Folder { children: kids, .. } => {
                let p = find_folder_prefix(kids);
                if !p.is_empty() {
                    return p;
                }
            }
