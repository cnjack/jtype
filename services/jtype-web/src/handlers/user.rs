...
pub async fn get_profile(
    ...
    Ok(Json(ProfileResponse {
        ...
        enabled: row.try_get::<Option<String>, _>("disabled_at").unwrap_or(None).is_none(),
...
pub async fn update_site_settings(
    ...
    if let Some(ref site_title) = payload.site_title {
        if site_title.trim().is_empty() || site_title.len() > 255 {
            return Err(AppError::BadRequest("site title must be 1-255 chars".to_string()));
        }
...
pub async fn my_storage(
    ...
    let budget: i64 = sqlx::query("SELECT COALESCE(storage_budget_bytes, 1073741824) AS budget FROM users WHERE id = ?")
        .bind(&user.id)
        .fetch_one(&state.pool)
        .await?
        .try_get("budget")?;
