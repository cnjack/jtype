...
pub async fn save_document(
    ...
    match saved {
        SaveDocumentOutcome::Saved(doc) => Ok(Json(doc)),
        SaveDocumentOutcome::Conflict(c) => Err(AppError::BadRequest(format!("document conflict: {}", c.conflict_id))),
    }
}
...
pub async fn update_status(
    ...
    let status = match payload.status.trim().to_ascii_lowercase().as_str() {
        ...
        "published" => "published",
        "archived" => "archived",
        _ => return Err(AppError::BadRequest("status must be draft, published, or archived".to_string())),
    };
...
pub async fn save_document_version(
    ...
    if let Some(row) = current {
        ...
        let saved = save_merged_document(
            pool, workspace_id, user, &document_id, &relative_path,
            &title, status, &payload.content, parent_version_id.as_deref(), None, source,
        )
        .await?;
    ...
    .execute(&mut *tx)
    .await?;
    insert_document_version(&mut tx, &version_id, workspace_id, &document_id, None, None, &user.id, source, &content_hash, &payload.content).await?;
    tx.commit().await?;
...
pub async fn save_merged_document(
    ...
    .execute(&mut *tx)
    .await?;
    insert_document_version(&mut tx, &version_id, workspace_id, document_id, parent_version_id, base_version_id, &user.id, source, &content_hash, content).await?;
    tx.commit().await?;
